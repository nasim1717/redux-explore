...
export default function Modal({ open, control }) {
  ...
  const [editConversation, { isSuccess: isEditConverSuccess }] = useEditConversationMutation();
  ...
  // listen conversation add/edit succes
  useEffect(() => {
    if (isAddConversationSuccess || isEditConverSuccess) {
      control();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAddConversationSuccess, isEditConverSuccess]);
      ...
      <>
        ...
        <div className="rounded w-[400px] lg:w-[600px] space-y-8 bg-white p-10 absolute top-1/2 left-1/2 z-20 -translate-x-1/2 -translate-y-1/2">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-6">
            ...
            {responseError && <Error message="You can not send message to yourself!" />}
          </form>
        </div>
      </>
    )
